...
export async function getMyFriends(req, res) {
    try {
        ...
        .populate('friends','fullName  profilePic nativeLanguage learningLanguage','-password -email');
...
export async function acceptFriendRequest(req,res){
    try {
        ...
        if(friendRequest.receiver.toString() !== req.user.id){
            return res.status(403).json({message:"You are not authorized to accept the friend request"})
        }
